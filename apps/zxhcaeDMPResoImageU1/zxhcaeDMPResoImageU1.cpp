...
bool ResoResaTraverseIntensityU1(const  short *sImgData, const  short *sWinImgData,const zxhImageInfo *pImgInfo,float m_fMean,float m_fStdDev,const short *sVslsData, const short *sWinVslsData,const short *sWinVslsxData,const short *sWinVslsyData,const short *sWinVslszData,const zxhImageInfo *pVslsInfo,int SearchiRange[4],zxhImageDataT<short> *imgReadNewRaw,zxhImageDataT<short> *imgReadNewVsls,zxhImageDataT<short> *imgReadNewVslsx,zxhImageDataT<short> *imgReadNewVslsy,zxhImageDataT<short> *imgReadNewVslsz)
		...
		for(int iz=0;iz<ImgNewRawSize[2];++iz)
			for(int iy=0;iy<ImgNewRawSize[1];++iy)
				for(int ix=0;ix<ImgNewRawSize[0];++ix)
				{
					...
					double dNewVslxVal=0;double dNewVslyVal=0;double dNewVslzVal=0;
						...
						for(locali[1]=itoImg[1]-SearchiRange[1];locali[1]<=itoImg[1]+SearchiRange[1];locali[1]++)
							for(locali[0]=itoImg[0]-SearchiRange[0];locali[0]<=itoImg[0]+SearchiRange[0];locali[0]++)
							{
								...
								if (dvmultiplys<LOCALMIN)
								{
									...
									dNewVslxVal=sWinVslsxData[newiz * nImgWY * nImgWX + newiy* nImgWX +newix];
									dNewVslyVal=sWinVslsyData[newiz * nImgWY * nImgWX + newiy* nImgWX +newix];
									dNewVslzVal=sWinVslszData[newiz * nImgWY * nImgWX + newiy* nImgWX +newix];
							...
							imgReadNewVslsx->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewVslxVal);
							imgReadNewVslsy->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewVslyVal);
							imgReadNewVslsz->SetImageData(iz * ImgNewRawSize[1] * ImgNewRawSize[0] + iy* ImgNewRawSize[0] +ix,dNewVslzVal);
...
int main(int argc, char *argv[])//without intensity nomalization
{
	...
	string strFileNameVslsx=argv[3];
	string strFileNameVslsy=argv[4];
	string strFileNameVslsz=argv[5];
	char *chResoRawName = argv[6];
	char *chResoVslsName = argv[7];
	char * chResoVslsxName=argv[8];
	char * chResoVslsyName=argv[9];
	char * chResoVslszName=argv[10];
	char *chAtlsMeanStdDevFileName = argv[11];
	char *chUnseenMeanStdDevFileName = argv[12];
	...
	float fNewImgSpacing[]={1,1,1,1};//Add by JDQ
	float fOldImgSpacing[]={1,1,1,1};
	float fImgExtend[ZXH_ImageDimensionMax]={1,1,1,1};
	float div[ZXH_ImageDimensionMax] = {1,1,1,1};
	...
	*/
	...
	imgReadNewRaw->NewImage(imgReadRaw.GetDimension(),size,fNewImgSpacing, imgReadRaw.GetImageInfo());
	imgReadNewVsls->NewImage(imgReadVsls.GetDimension(),size,fNewImgSpacing, imgReadVsls.GetImageInfo());
	imgReadNewVslsx->NewImage(imgReadVslsx.GetDimension(),size,fNewImgSpacing, imgReadVsls.GetImageInfo());
	imgReadNewVslsy->NewImage(imgReadVslsy.GetDimension(),size,fNewImgSpacing, imgReadVsls.GetImageInfo());
	imgReadNewVslsz->NewImage(imgReadVslsz.GetDimension(),size,fNewImgSpacing, imgReadVsls.GetImageInfo());
	// get the vesselness data 
	const short *sVslsData = imgReadVsls.GetImageData();	
	const short *sVslsxData = imgReadVslsx.GetImageData();
	const short *sVslsyData = imgReadVslsy.GetImageData();
	const short *sVslszData = imgReadVslsz.GetImageData();
	//get the image size
	int nImWX, nImWY, nImWZ, nImWT;
	imgReadRaw.GetImageSize(nImWX, nImWY, nImWZ, nImWT);
	//get the new resample image size
	...
	short *sWinVslsx = new short[nImWX * nImWY * nImWZ];
	short *sWinVslsy = new short[nImWX * nImWY * nImWZ];
	short *sWinVslsz = new short[nImWX * nImWY * nImWZ];
	short sRawMin = 0, sRawMax = 0;
	short sMolRawMin = 0, sMolRawMax = 0;
	...
	for (int i = 0; i < nImWX * nImWY * nImWZ; i++)
	{
		...
		//else
		sWinImg[i] = sImData[i];
		sWinVslsx[i]=sVslsxData[i];
		sWinVslsy[i]=sVslsyData[i];
		sWinVslsz[i]=sVslszData[i];
	...
	ResoResaTraverseIntensityU1(sImData,sWinImg, imgReadRaw.GetImageInfo(),fUnseenCAMean, fUnseenCAStdDev, \
		sVslsData,sWinVsls,sWinVslsx,sWinVslsy,sWinVslsz, imgReadVsls.GetImageInfo(),SearchRange,imgReadNewRaw,imgReadNewVsls,imgReadNewVslsx,imgReadNewVslsy,imgReadNewVslsz);
	...
	zxh::SaveImage(vslx,chResoVslsxName);
	const zxhImageDataT<short> *vsly=imgReadNewVslsy;
	zxh::SaveImage(vsly,chResoVslsyName);
	const zxhImageDataT<short> *vslz=imgReadNewVslsz;
	zxh::SaveImage(vslz,chResoVslszName);
	delete[]sWinImg;
	delete[]sWinVsls;
	delete[]sWinVslsx;
	delete[]sWinVslsy;
	...
	delete[] imgReadNewRaw;
	delete[] imgReadNewVsls;
	delete[] imgReadNewVslsx;
 delete[] imgReadNewVslsy;
	delete[] imgReadNewVslsz;
}
