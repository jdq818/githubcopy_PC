...
if( argc < 12 )
	{
		std::cerr << "usage: " << std::endl;
		std::cerr << "zxhcaedmp_1.0_wmmllT	imageraw (.nii)  vesselness	(.nii)	atlas_vesselx	results	atlas_vessel	\
				detected-ostial atlas_mean	unseen_mean	modelpointsintensity linemask	segment_num maxvectordist" << std::endl;
		return -1;
	}
...
int nMaxSgmtNum =atoi(argv[11]);
char *chREFEndFileName =argv[12];//"F:/Coronary_0/Coronary_Niessen/ProcessByLL/training/dataset07/vessel1/pointE.txt";
...
*/
	...
	{			
		...
		//nItrNum = iMinPathModelCrct->ModifiedFastMarchingEvolutiondontMoveModel(nMaxItrNum,nItrNumSum,sWinImg);
		//nItrNum = iMinPathModelCrct->ModifiedFastMarchingEvolutiondontMoveModelWithLineMask(nMaxItrNum,nItrNumSum,sWinImg,sLineMaskData);
		nItrNum =iMinPathModelCrct->ModifiedFastMarchingEvolutiondontMoveModelwithFindMinPathPointCwithMaskMLLSM(nMaxItrNum,nItrNumSum,sWinImg,imgReadRaw.GetImageInfo(),nStartPoint,2000,sLineMaskData);
		//nItrNum =iMinPathModelCrct->ModifiedFastMarchingEvolutiondontMoveModelwithMask(nMaxItrNum,nItrNumSum,sWinImg,imgReadRaw.GetImageInfo(),nStartPoint,2000,sLineMaskData);
		nItrNumSum=nItrNumSum+nItrNum;
