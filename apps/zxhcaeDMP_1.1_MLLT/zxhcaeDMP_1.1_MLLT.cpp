...
float fmaxveclength=atof(argv[11]);
...
//char *chREFFileName = "F:/Coronary_0/Exp6_detect_ostia/Ostia0007To00007/dataset00_Meanostium_right.txt";//"F:/Coronary_0/Coronary_Niessen/ProcessByLL/training/dataset00/rightostium_world.txt";//rightostium while vessel0;others leftostium
...
*/
	...
	//ReadPointTxt(chAtlasCAFileName, vAtlasPoints);
...
// get end point
	...
	// get current time(unit: s)
	int nET = GetTime();
	int nMaxItrNum = 100000;
	int nMaxSumItrNum=400000;
	int nSaveItrNum=200;
	int nItrNum=0;
	int nItrNumSum=0;
	...
	// set the first mean and std-dev of Unseen Image
	iMinPathModelCrct->SetMeanStdDev(fUnseenCAMean, fUnseenCAStdDev);
	//set the maximum lenghth for the model vec
	iMinPathModelCrct->SetMaxVeclenth(fmaxveclength);
